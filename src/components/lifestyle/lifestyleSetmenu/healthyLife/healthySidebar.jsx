import React from "react";
import Paginate from "../../../catepagerightbar/Paginate";
import "../../../technology/technology.css";

export default function HealthySidebar() {
  return (
    <div className="sidebar">
      <div className="cate-main">
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo1.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo2.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo3.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo4.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo5.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo6.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo1.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo3.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo2.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <div className="postCate">
          <div className="Cate_img">
            <img
              src="./images/technology/computerpage/computerinfo4.jpg"
              alt=""
            />
            <p className="Catebutton cateTechnology">Technology</p>
          </div>
          <div className="postCate_info">
            <h4>18 Top Fall Fashion Trends from New York Fashion</h4>
            <p>
              All of the Best Looks From New York Fashion Week Fall/Winter
              2021.…
            </p>
            <div className="postbuttom">
              <div className="postman">
                <img src="./images/homeimgs/viedo4.jpg" alt="" />
                <span className="profileName">Paina Ta Kon</span>
                <span className="profileDate">20.3.2022</span>
              </div>
              <div className="posticon">
                <i className="uil uil-bookmark"></i>
              </div>
            </div>
          </div>
        </div>
        <Paginate />
      </div>
    </div>
  );
}
